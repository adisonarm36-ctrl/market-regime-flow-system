...
def test_dr_data_quality_summaries():
    ...
    # 1. Execution data summary test
    market_df = pd.DataFrame([
        {"Date": "2026-05-20", "DR_Ticker": "DEMO_DR", "Close": 10.0, "Volume": 100}
    ])
    bid_ask_df = pd.DataFrame([
        {"Date": "2026-05-20", "DR_Ticker": "DEMO_DR", "Bid": 9.9, "Ask": 10.1}
    ])
    
    eq_summary = summarize_dr_execution_quality_data(market_df, bid_ask_df)
    assert eq_summary.iloc[0]["dr_market_rows"] == 1
    assert eq_summary.iloc[0]["dr_bid_ask_rows"] == 1
    assert eq_summary.iloc[0]["unique_market_tickers"] == 1
    assert eq_summary.iloc[0]["unique_bid_ask_tickers"] == 1
    assert abs(eq_summary.iloc[0]["avg_spread_pct"] - 2.0) < 1e-5
    
    # Empty inputs test
    empty_eq = summarize_dr_execution_quality_data(None, None)
    assert empty_eq.iloc[0]["dr_market_rows"] == 0
    assert "missing_dr_market_data" in empty_eq.iloc[0]["warnings"]
    
    # 2. Fair value coverage summary test
    fv_inputs = pd.DataFrame([
        {"DR_Ticker": "DEMO_DR", "UnderlyingTicker": "UND_A", "FXPair": "USD/THB"}
    ])
    und_prices = pd.DataFrame([
        {"Date": "2026-05-20", "UnderlyingTicker": "UND_A", "Close": 1.0}
    ])
    fx_rates = pd.DataFrame([
        {"Date": "2026-05-20", "FXPair": "USD/THB", "Rate": 35.0}
    ])
    
    fv_summary = summarize_dr_fair_value_coverage(fv_inputs, und_prices, fx_rates)
    assert fv_summary.iloc[0]["fair_value_input_rows"] == 1
    assert fv_summary.iloc[0]["coverage_pct"] == 100.0
    assert fv_summary.iloc[0]["missing_underlying_tickers"] == ""
    
    # Missing underlying test
    fv_summary_missing = summarize_dr_fair_value_coverage(fv_inputs, None, None)
    assert fv_summary_missing.iloc[0]["coverage_pct"] == 0.0
    assert fv_summary_missing.iloc[0]["missing_underlying_tickers"] == "UND_A"
    assert "missing_underlying_prices" in fv_summary_missing.iloc[0]["warnings"]
    
    # 3. Tracking coverage summary test
    track_summary = summarize_dr_tracking_coverage(market_df, und_prices, fx_rates)
    assert track_summary.iloc[0]["dr_market_tickers"] == 1
    assert track_summary.iloc[0]["common_dates_count"] == 1
    assert track_summary.iloc[0]["tracking_coverage_status"] == "Partial"
